...
void Greedy::updatePolicy(const Evaluation &eval)
{
	...
	for (STATE_ID current_state = 0; current_state < num_of_states;
	     current_state++) {
		double best_reward_value;
		ACTION_ID best_action_idx = 0;

		// init reference value to max or min value to force
		// initalization
		...
		for (ACTION_ID action_idx = 0; action_idx < num_of_actions;
		     action_idx++) {
			total_reward = 0;
			for (STATE_ID state_idx = 0; state_idx < num_of_states;
			     state_idx++) {
				state_probability =
					model_.state_transition_matrix(
						action_idx, current_state,
						state_idx);
				reward = model_.reward_matrix(
					action_idx, current_state, state_idx);
				total_reward =
					total_reward +
					state_probability *
						(reward + eval.getValueOfState(
								  state_idx));
			}

			if ((model_.optGoal == OPT_MAXIMIZE) &&
			    (total_reward > best_reward_value)) {
...
void Greedy::printPolicy() const
{
	for (STATE_ID state_idx = 0; state_idx < model_.num_of_states;
		...
		std::cout << "STATE :     " << std::setw(5) << state_idx
			  << "    ACTION: " << model_.action_strings[action_idx]
			  << std::endl;
	}
}

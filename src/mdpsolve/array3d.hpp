...
template <typename T> class Array3d
{
	...
	Array3d(std::size_t x_length, std::size_t y_length,
		std::size_t z_length)
	...
	const T &operator()(const std::size_t x, const std::size_t y,
			    const std::size_t z) const
	{
		if ((x >= (xdim_) || y >= (ydim_) || z >= (zdim_)) ||
		    (length_ == 0)) {
			throw std::out_of_range(
			    "bad index when accessing Array3d object");
		} else {
			return data_[z + y * ydim_ +
				     x * ydim_ * zdim_]; // no boundary check
							 // here, faster
		}
	}
